...
class projectClass(Persistent):
	...
	def ImportTaskmonitor(self, file, addmode):
		...
		try:
			...
			print "Drmaa function unable, please contact the Administrator!"
		finally:
			pass
...
class jobClass(Persistent):
	...
	def Submit(self, DrmaaSessionObj):
		...
		remoteCommand = self.FileName
		workingDirectory = os.path.dirname(self.FileName)
		if self.param_q:
		...
		#LZX
		nativeSpecification = "-b no -shell yes -l vf=" + str(self.memRequestedG) + "G" + " -l num_proc=" + str(self.num_proc) + " -P " + self.projectObj.param_P + " -q " + self.projectObj.param_q + " " + self.Parameter
		...
		for scriptfile in os.listdir(workingDirectory):
			pmatch = re.match(r'{0}.e(\d+)'.format(jobscript), scriptfile)
		...
		if previd:
			...
			try:
				...
				try:
					qsub_cmd = "qsub -wd %s %s %s" % (workingDirectory, nativeSpecification, remoteCommand)
