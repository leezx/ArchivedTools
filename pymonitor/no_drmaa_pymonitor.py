			...
			for jobstring in joblist:
				...
				if not jobroot.has_key(jobarray[0]):
					...
					if len(jobarray) == 1:
						...
						#LZX
						jobroot[jobarray[0]].num_proc = ConfigFileObj.getHandle().getint('base', 'num_proc')
					else:
						#get memory limit
						...
						if len(jobarray) == 3:
							...
							# LZX
							pmatch_q3 = re.match(r'(\d+)cpu', jobarray[2])
							if pmatch_q3:
								jobroot[jobarray[0]].num_proc = int(pmatch_q3.group(1))
...
class jobClass(Persistent):
	...
	def Submit(self, DrmaaSessionObj):
		...
		#LZX
		#jt.nativeSpecification = "-b no -shell yes -l vf=" + str(self.memRequestedG) + "G" + " -l num_proc=" + str(self.num_proc) + " -P " + self.projectObj.param_P + " -q " + self.projectObj.param_q + " " + self.Parameter
		jt.nativeSpecfification = "-b no -shell yes -l vf=" + str(self.memRequestedG) + "G" + " -l num_proc=" + str(self.num_proc) + " -q " + self.projectObj.param_q + " " + self.Parameter
		...
		if previd:
			...
			print (jt.workingDirectory, jt.nativeSpecification, jt.remoteCommand)
			try:
				...
				try:
					qsub_cmd = "qsub -wd %s %s %s" % (jt.workingDirectory, jt.nativeSpecification, jt.remoteCommand)
...
class directExec(object):
	...
	def progexec(self, file, binary, stream, p, q, resource):
		...
		#parameter = parameter + " -shell yes -l " + resource  + " -q " + q
...
def projectimport():
	...
	remove_symlink()
	# print(optlist.opt_q)
	logging.info("optlist.opt_q : %s, optlist.opt_P : %s" % (optlist.opt_q, optlist.opt_P))
	if not optlist.opt_p:
		print "project name must be supplied."
		return
...
if __name__=='__main__':
	...
	logging.info("optlist.opt_q : %s, optlist.opt_P : %s" % (optlist.opt_q, optlist.opt_P))
	# confighandle = ConfigFileObj.getHandle()
	ConfigFileObj.cf.set('base', 'defaultq', optlist.opt_q)
	ConfigFileObj.Update()
